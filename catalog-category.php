...
if (!$category) { header('Location: ' . $base . '/index.php'); exit; }
...
?>
<style>
.cc-hero{padding:120px 0 60px;background:linear-gradient(135deg,#051b35 0%,#0e3c7d 60%,#1a5096 100%);position:relative;overflow:hidden;}
.cc-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:24px;}
.cc-card{background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e8eef5;text-decoration:none;box-shadow:0 2px 14px rgba(14,60,125,.06);transition:transform .25s,box-shadow .25s;display:block;}
.cc-card:hover{transform:translateY(-5px);box-shadow:0 10px 32px rgba(14,60,125,.14);}
.cc-card img{width:100%;height:240px;object-fit:cover;display:block;}
.cc-card-title{font-family:'Montserrat',sans-serif;font-size:14px;font-weight:700;color:#051b35;line-height:1.4;padding:16px 18px;}
.cc-tag{position:absolute;top:12px;left:12px;background:#0e3c7d;color:#fff;font-size:10px;font-weight:700;padding:4px 12px;border-radius:3px;font-family:'Montserrat',sans-serif;letter-spacing:.8px;text-transform:uppercase;}
@media(max-width:991px){.cc-grid{grid-template-columns:repeat(2,1fr);}}
@media(max-width:576px){.cc-grid{grid-template-columns:1fr;}}
</style>

<div class="cc-hero">
    <?php if(!empty($category['image'])):?>
    <div style="position:absolute;inset:0;background:url('<?php echo htmlspecialchars($category['image'],ENT_QUOTES,'UTF-8');?>') center/cover;opacity:.12;"></div>
    <?php endif;?>
...
<section style="padding:60px 0 90px;background:#f8f9fb;">
    <div class="container">
        <div style="font-size:12px;color:#6c757d;margin-bottom:32px;">
        ...
        <div class="cc-grid">
            <?php foreach($items as $it):
                $img = !empty($it['main_image'])
                    ? htmlspecialchars($it['main_image'],ENT_QUOTES,'UTF-8')
                    : $fallback;
