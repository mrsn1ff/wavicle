...
$stmt=$pdo->prepare("SELECT ci.*,cc.name as cat_name FROM catalog_items ci JOIN catalog_categories cc ON cc.id=ci.category_id WHERE ci.id=? AND ci.type='product'");
$stmt->execute([$id]); $item=$stmt->fetch(); if(!$item){header('Location: index.php');exit;}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!$errors){
        ...
        $newImg=handleImageUpload('main_image','catalog/products',$item['main_image']);
...
$adminPageTitle='Edit Product'; $adminActivePage='cat_products';
...
function refreshLabels(){
    document.querySelectorAll('.sec-block').forEach(function(b,i){
        var l=b.querySelector('.sec-label'); if(l) l.textContent='Section '+(i+1);
    });
}
...
<div class="wv-page-header">
    <div><h1>Edit Product</h1><div class="wv-breadcrumb"><a href="../../index.php">Dashboard</a> / <a href="index.php?cat=<?php echo $item['category_id'];?>">Products</a> / Edit</div></div>
...
<div class="wv-card" style="margin-bottom:20px;">
    ...
    <div class="wv-card__body">
        <div class="wv-form-row">
            <div class="wv-form-group"><label class="wv-label">Product Title <span class="wv-required">*</span></label><input type="text" name="title" class="wv-input" value="<?php echo e($data['title']);?>" required/></div>
            <div class="wv-form-group"><label class="wv-label">Category <span class="wv-required">*</span></label>
                <select name="category_id" class="wv-select" required><option value="">— Select —</option><?php foreach($categories as $cat):?><option value="<?php echo $cat['id'];?>" <?php echo $data['category_id']==$cat['id']?'selected':'';?>><?php echo e($cat['name']);?></option><?php endforeach;?></select></div>
        </div>
        <div class="wv-form-row">
...
<div style="display:flex;gap:12px;margin-bottom:40px;">
    <button type="submit" class="wv-btn wv-btn-success" style="padding:13px 36px;font-size:14px;"><i class="fa fa-save"></i> Update Product</button>
