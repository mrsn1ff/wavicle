...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        $img = handleImageUpload('main_image', 'catalog/services');
        $pdo->prepare('INSERT INTO catalog_items (category_id,type,title,slug,main_image,description,sort_order,status) VALUES (?,?,?,?,?,?,?,?)')
            ->execute([$data['category_id'], 'service', $data['title'], $data['slug'], $img ?: '', $data['description'], $data['sort_order'], $data['status']]);
        ...
        setFlash('success', 'Service "' . $data['title'] . '" saved successfully.');
...
<div class="wv-page-header">
    <div>
        <h1>Add Service</h1>
        <div class="wv-breadcrumb"><a href="../../index.php">Dashboard</a> / <a href="index.php">Services</a> / Add</div>
    </div>
...
<?php if (empty($categories)): ?>
    <div style="background:#fff3cd;border:1px solid #ffc107;color:#856404;padding:16px 20px;border-radius:8px;margin-bottom:20px;">
        <i class="fa fa-triangle-exclamation"></i> <strong>No service categories found.</strong>
        <a href="../categories/create.php?type=service" style="color:#856404;font-weight:700;margin-left:8px;">Create a category first →</a>
    </div>
<?php endif; ?>
...
<form method="POST" enctype="multipart/form-data" onsubmit="syncCK()">
        ...
        <div class="wv-card__body">
            <div class="wv-form-row">
                <div class="wv-form-group">
                    <label class="wv-label">Service Title <span class="wv-required">*</span></label>
            ...
            <div class="wv-form-row">
                <div class="wv-form-group">
                    ...
                    <small style="color:#6c757d;font-size:11px;margin-top:6px;display:block;">Recommended: 1080×1080px, JPG/PNG/WebP</small>
                </div>
                <div class="wv-form-group">
            ...
            <div class="wv-form-group">
                <label class="wv-label">Description <span class="wv-required">*</span>
                    <small style="font-weight:400;text-transform:none;letter-spacing:0;color:#adb5bd;"> — shown beside main image on service page</small>
                </label>
    ...
    <div style="display:flex;gap:12px;margin-bottom:40px;">
        <button type="submit" class="wv-btn wv-btn-success" style="padding:13px 36px;font-size:14px;"><i class="fa fa-save"></i> Save Service</button>
