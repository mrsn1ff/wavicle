...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['delete_id'])){
    ...
    $r=$pdo->prepare("SELECT main_image FROM catalog_items WHERE id=? AND type='service'"); $r->execute([$id]);
    $img=$r->fetchColumn();
    ...
    setFlash('success','Service deleted.'); header('Location: index.php'.($catId=$_POST['cat_id']??0?'?cat='.(int)$_POST['cat_id']:'')); exit;
}

$catId=(int)($_GET['cat']??0);
$allCats=$pdo->query("SELECT * FROM catalog_categories WHERE type='service' AND status=1 ORDER BY sort_order ASC, name ASC")->fetchAll();

if($catId){
    $stmt=$pdo->prepare("SELECT ci.*,cc.name as cat_name FROM catalog_items ci JOIN catalog_categories cc ON cc.id=ci.category_id WHERE ci.category_id=? AND ci.type='service' ORDER BY ci.sort_order ASC,ci.id ASC");
    $stmt->execute([$catId]);
} else {
    $stmt=$pdo->query("SELECT ci.*,cc.name as cat_name FROM catalog_items ci JOIN catalog_categories cc ON cc.id=ci.category_id WHERE ci.type='service' ORDER BY cc.sort_order ASC,ci.sort_order ASC,ci.id ASC");
}
$items=$stmt->fetchAll();

$adminPageTitle='Services'; $adminActivePage='cat_services';
include __DIR__.'/../../includes/header.php';
?>
<div class="wv-page-header">
    <div><h1>Services</h1><div class="wv-breadcrumb"><a href="../../index.php">Dashboard</a> / Services</div></div>
    <a href="create.php<?php echo $catId?'?cat='.$catId:'';?>" class="wv-btn wv-btn-primary"><i class="fa fa-plus"></i> Add Service</a>
</div>
<?php renderFlash();?>
<div style="display:flex;gap:8px;margin-bottom:24px;flex-wrap:wrap;align-items:center;">
    ...
    <?php endforeach;?>
    <a href="../categories/index.php?type=service" class="wv-btn wv-btn-secondary wv-btn-sm" style="margin-left:auto;"><i class="fa fa-folder-open"></i> Manage Categories</a>
</div>
<div class="wv-card">
    <div class="wv-card__header"><span class="wv-card__title">All Services (<?php echo count($items);?>)</span></div>
