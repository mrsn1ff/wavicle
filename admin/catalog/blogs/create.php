...
$errors=[];
$data=['title'=>'','slug'=>'','description'=>'','author'=>'Admin','sort_order'=>0,'status'=>1];
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!$errors){
        ...
        $img=handleImageUpload('main_image','catalog/blogs');
        $pdo->prepare('INSERT INTO blogs (title,slug,main_image,description,author,status) VALUES (?,?,?,?,?,?)')
            ->execute([$data['title'],$data['slug'],$img?:'',$data['description'],$data['author'],$data['status']]);
        ...
        foreach($sections as $sec){
            ...
            if(!$c) continue;
            $pdo->prepare('INSERT INTO blog_sections (blog_id,heading,content,sort_order) VALUES (?,?,?,?)')->execute([$blogId,$h,$c,$order++]);
        }
        setFlash('success','Blog "'.$data['title'].'" created.');
        header('Location: index.php'); exit;
    }
}
$adminPageTitle='Add Blog'; $adminActivePage='cat_blogs';
...
function addSection(h,c){
    ...
    var html='<div class="sec-block" id="sb_'+idx+'" style="border:1px solid #e8eef5;border-radius:10px;margin:16px 20px;overflow:hidden;">'
        ...
        +'<input type="text" name="sections['+idx+'][heading]" class="wv-input" value="'+(h||'')+'" placeholder="e.g. Introduction, Key Points..."/></div>'
    ...
    initCK(tid); refreshLabels();
}
function delSec(idx){
    if(!confirm('Delete this section?')) return;
    if(ckInstances['sec_'+idx]) ckInstances['sec_'+idx].destroy();
    delete ckInstances['sec_'+idx];
    var el=document.getElementById('sb_'+idx); if(el) el.remove(); refreshLabels();
}
function refreshLabels(){
    document.querySelectorAll('.sec-block').forEach(function(b,i){
        var l=b.querySelector('.sec-label'); if(l) l.textContent='Section '+(i+1)+(i===0?' — shown beside main image':'');
    });
}
...
<div class="wv-page-header">
    <div><h1>Add Blog</h1><div class="wv-breadcrumb"><a href="../../index.php">Dashboard</a> / <a href="index.php">Blogs</a> / Add</div></div>
    ...
    <div class="wv-card__body">
        <div class="wv-form-row">
            <div class="wv-form-group">
                ...
                <input type="text" name="title" class="wv-input" value="<?php echo e($data['title']);?>" placeholder="Blog title" required oninput="document.getElementById('slug').value=makeSlug(this.value)"/>
            </div>
            <div class="wv-form-group">
                <label class="wv-label">Author</label>
                <input type="text" name="author" class="wv-input" value="<?php echo e($data['author']);?>" placeholder="Author name"/>
            </div>
        </div>
        <div class="wv-form-row">
            <div class="wv-form-group">
                ...
                <small style="color:#6c757d;font-size:11px;margin-top:4px;display:block;">Used in URL: /news/{slug}</small>
            </div>
            <div class="wv-form-group">
...
<div class="wv-card" style="margin-bottom:20px;">
    <div class="wv-card__header">
        <span class="wv-card__title"><i class="fa fa-list-ul" style="color:#59b5e8;margin-right:8px;"></i>Content Sections</span>
        <small style="color:#6c757d;font-size:12px;">Each section = heading + paragraph</small>
    </div>
...
<div style="display:flex;gap:12px;margin-bottom:40px;">
    <button type="submit" class="wv-btn wv-btn-success" style="padding:13px 36px;font-size:14px;"><i class="fa fa-save"></i> Save Blog</button>
