...
$stmt=$pdo->prepare('SELECT * FROM services WHERE id=?');$stmt->execute([$id]);$service=$stmt->fetch();
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    $data['title']=trim($_POST['title']??'');$data['icon_class']=trim($_POST['icon_class']??'scubo-icon-scuba-diving');
    $data['description']=trim($_POST['description']??'');$data['sort_order']=(int)($_POST['sort_order']??0);$data['status']=isset($_POST['status'])?1:0;
    if(!$data['title'])$errors[]='Title is required.';if(!$data['description'])$errors[]='Description is required.';
    if(!$errors){
        $slug=$service['slug'];
        if($data['title']!==$service['title']){$slug=makeSlug($data['title']);$base=$slug;$i=1;while(true){$chk=$pdo->prepare('SELECT COUNT(*) FROM services WHERE slug=? AND id!=?');$chk->execute([$slug,$id]);if((int)$chk->fetchColumn()===0)break;$slug=$base.'-'.(++$i);}}
        $pdo->prepare('UPDATE services SET title=?,slug=?,icon_class=?,description=?,sort_order=?,status=? WHERE id=?')
            ->execute([$data['title'],$slug,$data['icon_class'],$data['description'],$data['sort_order'],$data['status'],$id]);
        setFlash('success','Service updated successfully.');header('Location: index.php');exit;
    }
}
$adminPageTitle='Edit Service';$adminActivePage='services';
include __DIR__ . '/../includes/header.php';
?>
<div class="wv-page-header">
    <div><h1>Edit Service</h1><div class="wv-breadcrumb"><a href="../index.php">Dashboard</a> / <a href="index.php">Services</a> / Edit</div></div>
...
<div class="wv-card"><div class="wv-card__header"><span class="wv-card__title">Edit: <?php echo e($service['title']);?></span></div><div class="wv-card__body">
<form method="POST">
    ...
    <div class="wv-form-group"><label class="wv-label">Slug</label><input type="text" class="wv-input" value="<?php echo e($service['slug']);?>" readonly style="background:#f4f6fb;color:#6c757d;font-size:12px;"/></div>
    ...
    <div style="display:flex;gap:12px;"><button type="submit" class="wv-btn wv-btn-success"><i class="fa fa-save"></i> Update Service</button><a href="index.php" class="wv-btn wv-btn-secondary">Cancel</a></div>
</form>
</div></div>
